...
describe('Lost Items Routes', () => {
  ...
  it('POST /api/v1/lost-items - should report a new lost item', async () => {
  ...
  it('PUT /api/v1/lost-items/:id - should update a lost item', async () => {
    const res = await request(app).put(`/api/v1/lost-items/${createdId}`).send(updatedItem);
    expect(res.status).toBe(200);
  ...
  it('DELETE /api/v1/lost-items/:id - should delete a lost item', async () => {
    const res = await request(app).delete(`/api/v1/lost-items/${createdId}`);
    expect(res.status).toBe(200);
